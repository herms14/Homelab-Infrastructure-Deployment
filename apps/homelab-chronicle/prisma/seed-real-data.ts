...
<h4>Prometheus Scrape Targets</h4>
<ul>
  <li>Prometheus (self)</li>
  <li>Proxmox VE nodes (via PVE Exporter)</li>
  <li>Synology NAS (via SNMP Exporter)</li>
  <li>Traefik (192.168.40.20:8082)</li>
...
<p>Requires special AppArmor config for Docker in LXC:</p>
<pre>services:
  glance:
...
<h4>Authentik SSO Integration</h4>
<p>Configured OAuth2 authentication via Authentik for secure access.</p>
...
<h4>GitLab Runner</h4>
<p>Deployed dedicated runner VM (192.168.40.24) for CI/CD job execution.</p>
...
<p>See <a href="https://glance.hrmsmrflrii.xyz">30 - LXC Migration Tutorial</a> for detailed steps.</p>`
  },
...
<h4>Switch Port Configuration</h4>
<p>Proxmox nodes configured with VLAN-aware bridges for VM network access.</p>`
  },
...
<h4>Proxmox Storage Configuration</h4>
<pre>pvesm add nfs VMDisks 192.168.20.31:/volume1/VMDisks
pvesm add nfs MediaFiles 192.168.20.31:/volume2/Proxmox-Media</pre>`
  },
...
<h4>Traefik Integration</h4>
<p>Traefik configured to send traces to OTEL Collector:</p>
<pre>tracing:
  otlp:
    http:
      endpoint: "http://192.168.40.13:4318/v1/traces"</pre>
...
<table>
  ...
  <tr><td>k8s-worker01</td><td>192.168.20.40</td><td>4 cores, 8GB RAM</td></tr>
  <tr><td>k8s-worker02</td><td>192.168.20.41</td><td>4 cores, 8GB RAM</td></tr>
  <tr><td>k8s-worker03</td><td>192.168.20.42</td><td>4 cores, 8GB RAM</td></tr>
  <tr><td>k8s-worker04</td><td>192.168.20.43</td><td>4 cores, 8GB RAM</td></tr>
  <tr><td>k8s-worker05</td><td>192.168.20.44</td><td>4 cores, 8GB RAM</td></tr>
  <tr><td>k8s-worker06</td><td>192.168.20.45</td><td>4 cores, 8GB RAM</td></tr>
</table>
...
<h4>Architecture</h4>
<p>Uses SSH for Proxmox operations, connects to nodes as root and VMs as hermes-admin.</p>

<h4>Discord Integration</h4>
<ul>
  <li>Bot Channel: #sentinel-bot</li>
  <li>Update Notifications: #container-updates</li>
  <li>Media Downloads: #media-downloads</li>
</ul>`
  },
...
├── consume/   # Drop folder for new docs
└── export/    # Backup exports</pre>`
  },
...
<ul>
  ...
  // ============================================
  {
    title: 'Technical Manual V5.4 - Complete Disaster Recovery Reference',
    date: new Date('2026-01-13'),
    category: 'documentation',
    source: 'manual',
    tags: JSON.stringify(['documentation', 'technical-manual', 'disaster-recovery', 'credentials', 'tutorials']),
    services: JSON.stringify(['Documentation']),
    content: `<h3>Complete Self-Contained Infrastructure Reference</h3>
<p>Expanded the Homelab Technical Manual to Version 5.4 with comprehensive credentials reference and step-by-step deployment tutorials, creating a complete disaster recovery reference.</p>

<h4>Document Evolution</h4>
<table>
  <tr><th>Version</th><th>Content</th></tr>
  <tr><td>V5.0</td><td>Initial comprehensive structure</td></tr>
  <tr><td>V5.1</td><td>Kubernetes installation guide</td></tr>
  <tr><td>V5.2</td><td>GitOps and Service Onboarding</td></tr>
  <tr><td>V5.3</td><td>PBS monitoring and backup documentation</td></tr>
  <tr><td>V5.4</td><td>Credentials Reference + Tutorials</td></tr>
</table>

<h4>New Sections Added</h4>
<ul>
  <li><strong>Credentials Reference</strong> (~220 lines)
    <ul>
      <li>Network Infrastructure (Omada, device passwords, ISP gateway)</li>
      <li>Proxmox Cluster (API tokens, PBS credentials)</li>
      <li>Authentik OAuth (Google, Immich, Grafana)</li>
      <li>Discord Bot tokens</li>
      <li>Arr Media Stack API keys</li>
      <li>WiFi SSID passwords</li>
      <li>OPNsense API credentials</li>
      <li>Application service passwords</li>
    </ul>
  </li>
  <li><strong>Tutorials Section</strong> (~520 lines)
    <ul>
      <li>Discord Bot Deployment Tutorial</li>
      <li>Immich Authentik SSO Tutorial</li>
      <li>LXC Migration Tutorial</li>
      <li>PBS Deployment Tutorial</li>
    </ul>
  </li>
</ul>

<h4>Purpose</h4>
<p>The manual now serves as a <strong>single point of reference</strong> for complete infrastructure rebuilding from scratch. All credentials, configurations, and step-by-step tutorials are embedded directly in the document.</p>

<h4>Total Size</h4>
<p>~3,800 lines of comprehensive documentation covering every aspect of the homelab infrastructure.</p>`
  }
]
